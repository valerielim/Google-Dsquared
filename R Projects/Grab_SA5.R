...
# Prep: Subset and transform data types
...
grab_afinn <- transform(grab_afinn, message_ID=as.numeric(message_ID))
...
grab_boxplot$mean <- round(grab_boxplot$mean, digits=2)
uber_boxplot$mean <- round(uber_boxplot$mean, digits=2)
grab_boxplot[is.na(grab_boxplot)] <- 0
uber_boxplot[is.na(uber_boxplot)] <- 0

# Boxplot per month
boxplot_g <- ggplot(grab_boxplot, aes(x=factor(Month_num), y=mean)) +
    xlab("Month") + ylab("Average Politeness") + 
    geom_boxplot()
boxplot_u <- ggplot(uber_boxplot, aes(x=factor(Month_num), y=mean)) +
    xlab("Month") + ylab("Average Politeness") + 
    geom_boxplot()
