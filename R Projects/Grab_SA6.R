...
uber <- read.csv(file.choose(), header=T, na.strings=c("", " ","NA"))
grab <- read.csv(file.choose(), header=T, na.strings=c("", " ","NA"))
...
print(plot_uber + 
    ...
    scale_y_continuous(labels = NULL, limits = c(-75, 50)) + xlab("") + 
    ylab("Grab  -  Uber") + 
...
grab_afinn <- grab_words %>%
    group_by(message_ID) %>%
    summarise(mean = mean(score, na.rm=TRUE)) # 2900
...
grab_afinn <- transform(grab_afinn, message_ID=as.numeric(message_ID)) # 1700
uber_afinn <- transform(uber_afinn, message_ID=as.numeric(message_ID))
grab <- left_join(grab, grab_afinn, by = "message_ID")
uber <- left_join(uber, uber_afinn, by = "message_ID")
grab[is.na(grab)] <- 0
uber[is.na(uber)] <- 0

grab <- subset(grab, select= -c(mean.x, mean.y, mean))
...
uber_lab <- uber[as.character(uber$sentiment)=="positive", ] #n=1,391
grab_lab <- grab[(grab$sentiment)!="", ] #n=1,237
classifier_g <- naiveBayes(grab_lab[,c(11:16)], grab_lab[,5])
classifier_u <- naiveBayes(uber_lab[,c(11:16)], uber_lab[,5])
